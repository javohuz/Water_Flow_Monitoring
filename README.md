...
## Tilni almashtirish (frontend):
Saytga oddiy tugma orqali tilni o'zgartirishni qo‘shish mumkin. Masalan:

```html
<button onclick="changeLanguage('uz')">UZ</button>
<button onclick="changeLanguage('en')">EN</button>
<p id="desc">Suv oqimini kuzatish tizimi</p>

<script>
  const translations = {
    uz: "Suv oqimini kuzatish tizimi",
    en: "Water flow monitoring system"
  };

  function changeLanguage(lang) {
    document.getElementById("desc").innerText = translations[lang];
  }
</script>
```
